...
const BookMeeting = () => {
   const navigate = useNavigate();
    ...
    const handleSubmit = async () => {  
        ...
        setTimeout(() => {
            navigate(`/meeting?company=${companyNameRef.current?.value}&email=${emailRef.current?.value}`, {state: {email: emailRef.current?.value, company: companyNameRef.current?.value}})
        }, 500);
    };
